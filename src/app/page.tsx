...
import { BasePageConfig, PublicationPageConfig, TextPageConfig, CardPageConfig, TimelinePageConfig } from '@/types/page';
...
type PageData =
  ...
  | { type: 'timeline', id: string, config: TimelinePageConfig };
  ...
  const processSections = (sections: SectionConfig[]) => {
    return sections.map((section: SectionConfig) => {
      switch (section.type) {
        ...
        case 'publications': {
          const bibtex = getBibtexContent(section.source || 'publications.bib');
  ...
  if (enableOnePageMode) {
    pagesToShow = config.navigation
      ...
      .map(item => {
        ...
        } else if (pageConfig.type === 'timeline') {
          return {
            type: 'timeline',
            id: item.target,
            config: pageConfig as TimelinePageConfig
          } as PageData;
        }
        return null;
      })
